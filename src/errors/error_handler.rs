use super::{token::{ Token, TokenType }, StmtErrors};
        ...
        for token in &self.lex_errors{
            if let TokenType::Error(err_type) = &token.class {
                eprintln!( "{}",
                    ...
                    err_type.get_message(), token.line, token.start).bright_red()
                );
        ...
        for error in errors.errors.iter(){
            ...
            eprintln!( "{}",
                format!(
                    ...
                    error.get_message(), error_position.0, error_position.1
                )
                .bright_red()
            );
            self.print_code_snippet(
                error_position.0, error_position.1, 1
            )
        }
    }
    ...
    fn print_code_snippet(&self, line: u32, pos: u32, surround_lines: u32){
        ...
        for code_line in self.source.lines(){
            if current_line == line{
                eprintln!("{}{}", (current_line.to_string() + " | ").bright_cyan(), code_line);
                ...
                eprintln!("{}{}", " | ".bright_cyan(), code_line.truecolor(150, 150, 150));
            }
            current_line += 1;
        }
        eprintln!("\n")
