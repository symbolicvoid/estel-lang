use super::errors::{ExpectType, ExprError, StmtError, StmtErrors};
use super::expr::*;
use super::stmt::*;
use super::token::*;
...
impl<'a> Parser<'a> {
    ...
    pub fn make_statement(&mut self) -> Result<Stmt, StmtError> {
        match &self.get_current_token().class {
            ...
            TokenType::Ident(
                name, /*Borrow checker won't let me use the name here so ¯\_(ツ)_/¯*/
            ) => {
                let name = name.clone();
            ...
            _ => Err(StmtError::InvalidStartToken(
                self.get_current_token().to_owned(),
            )),
        }
    }
            ...
            Ok(expr) => {
                match expr {
                    None => Err(
                        ...
                        StmtError::InvalidExpression(ExprError::ExpectTokenError(
    ...
    fn expr(&mut self) -> Result<Option<Expr>, ExprError> {
        match self.get_current_token().class {
            ...
            TokenType::Lparen => {
                ...
                match paren_expr {
                    None => Err(ExprError::ExpectTokenError(
                        ExpectType::Expression,
                        self.get_current_token().to_owned(),
                    )),
                    Some(expr) => {
